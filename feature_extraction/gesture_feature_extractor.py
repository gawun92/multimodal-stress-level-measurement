...
def process_all(split: str = "train"):
    ...
    print(f"[gesture_feature_extractor] Found {len(mp4_files)} .mp4 files in '{split}' split")
    ...
    for mp4_path in tqdm(mp4_files, desc=f"  [{split}] Hand Landmarks"):
        ...
        out_path = os.path.join(subject_out_dir, f"{task}_gesture.npy")
        ...
        try:
            landmarks = extract_hand_landmarks(str(mp4_path))
